...
def create_reagent(payload: ReagentCreate, db: Session = Depends(get_db)) -> HTMLResponse:
...
def list_reagents(db: Session = Depends(get_db)) -> HTMLResponse:
...
def get_reagent(reagent_id: int, db: Session = Depends(get_db)) -> HTMLResponse:
...
def update_reagent(
    reagent_id: int, payload: ReagentUpdate, db: Session = Depends(get_db)
) -> HTMLResponse:
...
def delete_reagent(reagent_id: int, db: Session = Depends(get_db)) -> HTMLResponse:
