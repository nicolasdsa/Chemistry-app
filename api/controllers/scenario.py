...
def create_scenario(payload: ScenarioCreatePayload, db: Session = Depends(get_db)) -> HTMLResponse:
...
def list_scenarios(db: Session = Depends(get_db)) -> HTMLResponse:
    scenarios = scenario_service.list_scenarios(db)
    data = [ScenarioRead.model_validate(item).model_dump() for item in scenarios]
    return HTMLResponse(content=json.dumps(data), status_code=200)


def get_scenario(scenario_id: int, db: Session = Depends(get_db)) -> HTMLResponse:
...
def update_scenario(
    scenario_id: int, payload: ScenarioUpdatePayload, db: Session = Depends(get_db)
) -> HTMLResponse:
...
def delete_scenario(scenario_id: int, db: Session = Depends(get_db)) -> HTMLResponse:
